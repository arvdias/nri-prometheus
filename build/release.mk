...
ifeq ($(PRERELEASE), true)
	...
	@$(GORELEASER_BIN) release --config $(CURDIR)/build/.goreleaser.yml --skip-validate --rm-dist
else
	@echo "===> $(INTEGRATION) === [release/build] build compiling all binaries"
	@$(GORELEASER_BIN) build --config $(CURDIR)/build/.goreleaser.yml --skip-validate --snapshot --rm-dist
endif
